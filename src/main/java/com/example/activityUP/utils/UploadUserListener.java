...
@Slf4j
public class UploadUserListener implements ReadListener<SaUser> {
     ...
     * 缓存的数据
     */
    private List<SaUser> cachedDataList = ListUtils.newArrayListWithExpectedSize(BATCH_COUNT);
    /**
     ...
     * @param saUserService
     */
    public UploadUserListener(SaUserService saUserService,SaUserRoleService saUserRoleService) {
        this.saUserService = saUserService;
        this.saUserRoleService = saUserRoleService;
    }
     ...
     */
    @Override
    public void invoke(SaUser data, AnalysisContext context) {
    ...
    private void saveData() {
        ...
        boolean update = saUserService.insertBatchOrUpdate(cachedDataList);
        if (update) {
            for (SaUser saUser: cachedDataList) {
                SaUserRole saUserRole = new SaUserRole();
                saUserRole.setUserId(saUser.getId());
